...
| T5 | Comparison table + evidence recommendation with FINAL DECISION OPEN: fill numbers from spike-floor.json into 5-user.md table + SCENE-CREATION § Floor/background; add ☐ co-decide checkpoint leaving the pick to Lucas; keep spike modules (branch stays unmerged) | .loop/floor-fog-spike/5-user.md, code/isoroll-content/SCENE-CREATION.md | SCENE-CREATION § Floor/background has the table + recommendation + ☐ co-decide; no winner wired into shipped code | medium | medium |

Loop mapping: L3 designs the fog-drive + oracle-schema seams (high tier owns the tricky fog-state driving); L4a writes T1–T3 unit tests + T4 spec failing; L4b implements T1–T3 green + T4 e2e green; L5 runs T4 for real + does T5; L6 commits branch, keeps `.loop/` (keep-trail=yes), leaves spike code in place (unmerged).
