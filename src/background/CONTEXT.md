...
- **GridConfig `renderGridConfig` hook**: fires after `#createPreview()` completes AND on Reset Changes (form re-render only — preview container persists). Arrow keys in GridConfig move background in projected grid axes (diagonal), not screen axes — see B22 in ISSUES.md.
