...
| `tile-drag.ts` | `DragState`, `HandleType`, `handlePositions()`, `projectDrag()`, `commitDrag()`. `commitDrag` calls `setFlag` for image handles (imgOffset/imgScale/imgYScale) and `document.update` for volume handles. `startPointerDrag` called with same `commitDrag` on both move and end — see B14 in ISSUES.md. |
...
| [`tile-gizmos-drag.ts`](tile-gizmos-drag.ts) | [`tile-gizmos-drag.d.ts`](tile-gizmos-drag.d.ts) | `swapSide`, `beginDrag`, `handlePointerDown` | Drag and swap-side interaction logic for VolumeGizmos handles. |
