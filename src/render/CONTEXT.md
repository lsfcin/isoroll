...
| [`iso-renderer-paint.ts`](iso-renderer-paint.ts) | [`iso-renderer-paint.d.ts`](iso-renderer-paint.d.ts) | `paintSpec` | iso-renderer-paint.ts — PIXI paint helpers for IsoRenderer. |
...
| [`iso-renderer.ts`](iso-renderer.ts) | [`iso-renderer.d.ts`](iso-renderer.d.ts) | `isoRendererSightRefresh`, `IsoRenderer` | Rendering façade — single entry point for all isoroll visuals. |
...
| [`iso-tile-debug-cells.ts`](iso-tile-debug-cells.ts) | [`iso-tile-debug-cells.d.ts`](iso-tile-debug-cells.d.ts) | `drawCellMarkers` | iso-tile-debug-cells.ts — per-cell slice marker rendering for debugSlices mode. |
| [`iso-tile-debug-paint.ts`](iso-tile-debug-paint.ts) | [`iso-tile-debug-paint.d.ts`](iso-tile-debug-paint.d.ts) | `makeText`, `drawCutLines`, `drawCutMarkers`, `drawFrontierDots`, `drawSliceOutlines` | iso-tile-debug-paint.ts — PIXI drawing helpers for the iso tile debug overlay. |
| [`iso-tile-debug.ts`](iso-tile-debug.ts) | [`iso-tile-debug.d.ts`](iso-tile-debug.d.ts) | `drawSliceDebug`, `maybeDrawSliceDebug`, `clearSliceDebug`, `clearAllSliceDebug`, `drawGridDebug` | iso-tile-debug.ts — visual debug overlay for iso tile slices |
...
| [`iso-tile-fog-sync.ts`](iso-tile-fog-sync.ts) | [`iso-tile-fog-sync.d.ts`](iso-tile-fog-sync.d.ts) | `syncAllTileFog` | iso-tile-fog-sync.ts — per-tile fog application broadcast to all slices (sightRefresh path). |
| [`iso-tile-geom.ts`](iso-tile-geom.ts) | [`iso-tile-geom.d.ts`](iso-tile-geom.d.ts) | `gridMetrics`, `tileSliceCount`, `computeSliceCuts`, `sliceStateChanged` | iso-tile-geom.ts — pure slice-cut math: grid metrics, frontier-face projection, structural diff. |
| [`iso-tile-rects.ts`](iso-tile-rects.ts) | [`iso-tile-rects.d.ts`](iso-tile-rects.d.ts) | `dumpStageMetrics`, `dumpTileRects`, `stageRect`, `assetOf`, `flagNumber` | dumpTileRects — where every tile's sprite ACTUALLY landed, as numbers. |
| [`iso-tile-renderer.ts`](iso-tile-renderer.ts) | [`iso-tile-renderer.d.ts`](iso-tile-renderer.d.ts) | `IsoTileRenderer` | IsoTileRenderer — depth-sorted iso-diagonal sliced tile sprites for the iso layer. |
| [`iso-tile-slice-build.ts`](iso-tile-slice-build.ts) | [`iso-tile-slice-build.d.ts`](iso-tile-slice-build.d.ts) | `getMesh`, `cloneSliceTexture`, `syncSlicePos`, `initSliceAnchor`, `buildSlice` | iso-tile-slice-build.ts — PIXI sprite construction for tile slices (split from iso-tile-geom). |
| [`iso-tile-state.ts`](iso-tile-state.ts) | [`iso-tile-state.d.ts`](iso-tile-state.d.ts) | `tileSlices`, `tileSliceCuts` | iso-tile-state.ts — shared registries for live tile slices and their slice-cut state. |
...
| [`iso-tile-zsync.ts`](iso-tile-zsync.ts) | [`iso-tile-zsync.d.ts`](iso-tile-zsync.d.ts) | `tileBand`, `syncTileZ`, `schedulePeerResync` | iso-tile-zsync.ts — per-slice z/visibility assignment and cross-tile peer band resync. |
...
| [`mesh-accessor.ts`](mesh-accessor.ts) | [`mesh-accessor.d.ts`](mesh-accessor.d.ts) | `meshQuad`, `boundsOf`, `meshBounds`, `MeshAccessor` | Typed, null-safe reader of tile/token mesh geometry. |
