...
| [`iso-tile-depth.ts`](iso-tile-depth.ts) | — | `frontierFaces`, `sliceDepthCell`, `tileSortBand`, `DEPTH_SCALE`, `TOKEN_BAND` | iso-tile-depth.ts — slice→cell depth assignment and zIndex banding for the iso sprite layer. |
| [`iso-tile-geom.ts`](iso-tile-geom.ts) | — | `gridMetrics`, `tileSliceCount`, `computeSliceCuts`, `sliceCellOverlaps`, `cloneSliceTexture` | iso-tile-geom.ts — grid metric, slice-cut, and sprite helpers for iso tile slicing. |
...
| [`iso-tile-zdebug.ts`](iso-tile-zdebug.ts) | — | `debugSlices`, `debugGrid`, `debugZOrder`, `scheduleDumpZOrder`, `consumeDumpFlag` | iso-tile-zdebug.ts — console/z-order debug switches and dump for iso tile slices. |
