...
| [`iso-tile-renderer.ts`](iso-tile-renderer.ts) | [`iso-tile-renderer.d.ts`](iso-tile-renderer.d.ts) | `IsoTileRenderer`, `_destroySlices`, `_buildAllSlices`, `_createTileSlices` | IsoTileRenderer — depth-sorted iso-diagonal sliced tile sprites for the iso layer. |
