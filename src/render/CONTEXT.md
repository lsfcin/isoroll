...
| [`iso-tile-debug-cells.ts`](iso-tile-debug-cells.ts) | — | `drawCellMarkers`, `_drawDot`, `_drawTriangle`, `_imgX` | iso-tile-debug-cells.ts — per-cell slice marker rendering for debugSlices mode. |
