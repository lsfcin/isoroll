...
| [`bake-frame.ts`](bake-frame.ts) | [`bake-frame.d.ts`](bake-frame.d.ts) | `cellToWorld` | The bake's grid, the module's world, and the one turn between them. |
...
| [`import-tiles.ts`](import-tiles.ts) | [`import-tiles.d.ts`](import-tiles.d.ts) | `manifestTileToData`, `footprintBox`, `spriteFlag` | Pure manifest-tile → Foundry Tile creation-data mapping (C1, T2). v14 center convention. |
| [`import-walls.ts`](import-walls.ts) | [`import-walls.d.ts`](import-walls.d.ts) | `manifestWallsToDefs`, `inCells`, `centreline` | Manifest wall[] → per-tile WallDef[] for createWallsFromDefs (C1/C2, T3). Bridges the |
