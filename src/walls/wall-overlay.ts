...
import { drawWallDisplay } from "./wall-overlay-ops";
import { drawWallSelect } from "./wall-overlay-select";
...
export class WallOverlay {
  ...
  static activate(): void {
    window.addEventListener("keydown", e => {
      if (e.altKey) {
        WallOverlay.setAltMode(true);
      }
    });
    window.addEventListener("keyup", e => {
      if (!e.altKey) {
        WallOverlay.setAltMode(false);
      }
    });
  }
  ...
  static show(tile: Tile): void {
    ...
    if (WallOverlay.selectTile === tile.id) {
      drawWallSelect(tile.document, tile.id, keys, refresh);
    } else {
      drawWallDisplay(tile, tile.id, keys);
    }
    _tileKeys.set(tile.id, keys);
  }

  static hide(tileId: string): void {
    if (_dragActive.has(tileId)) {
      return;
    }
    const tileKeySet = _tileKeys.get(tileId) ?? [];
    for (const k of tileKeySet) {
      IsoRenderer.clear(k);
    }
  ...
  static enterSelect(tile: Tile): void {
    WallOverlay.selectTile = tile.id;
    WallOverlay.show(tile);
  }
