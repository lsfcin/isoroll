...
| [`wall-overlay-ops.ts`](wall-overlay-ops.ts) | [`wall-overlay-ops.d.ts`](wall-overlay-ops.d.ts) | `addLineHover`, `addWallDblClick`, `addEndpointHandles`, `addSelectInteraction`, `scaleEndpoints` | Interactive helpers: endpoint handles, drag, select toggle. |
| [`wall-overlay.ts`](wall-overlay.ts) | [`wall-overlay.d.ts`](wall-overlay.d.ts) | `wallColor`, `WALL_COLORS` | PIXI overlay: shows linked walls when tile is selected, with select-mode picking. |
